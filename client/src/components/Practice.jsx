...
function Practice({ questions, jobDescription, companyName, roleTitle, techStack, jobDescriptionHash, studyTopics }) {
  ...
  // Auto-generate more questions if fewer than 10 — but only ONCE per job (persisted)
  useEffect(() => {
    ...
    if (sessionStorage.getItem(storageKey)) return // already ran this browser session
    setAutoGenerateAttempted(true)
    sessionStorage.setItem(storageKey, '1')
    handleGenerateMore(true) // silent mode
  ...
  return (
    <div className="practice-container">
      ...
      <div className="practice-mode-toggle">
        ...
        <button
          className={`mode-btn ${practiceMode === 'chat' ? 'active' : ''}`}
        ...
        <div style={{ marginLeft: 'auto', display: 'flex', gap: '8px', alignItems: 'center' }}>
          {practiceMode !== 'chat' && (
            <div className="smart-practice-wrapper">
              <button
                className={`mode-btn smart-btn ${smartMode ? 'active' : ''}`}
                onClick={() => setSmartMode(!smartMode)}
                title="Prioritizes questions you scored lowest on — your weakest topics first"
              >
                {smartMode ? 'Smart ON' : 'Smart'}
              </button>
            </div>
          )}
          <button
      ...
      {/* Smart Practice explanation banner */}
      {smartMode && practiceMode !== 'chat' && (
        <div className="smart-banner">
          <span className="smart-banner-text">
            Questions reordered by weakness — your hardest topics come first. Unattempted questions get top priority.
          </span>
        </div>
      )}
